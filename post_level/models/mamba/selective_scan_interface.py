...
if 'arg7' in (causal_conv1d_cuda.causal_conv1d_fwd.__doc__ or ''):
    # causal-conv1d >=1.6 moved to an in-place 8-arg kernel API; cpp_functions
    # exposes wrappers with the old 1.5.x 7-arg, tensor-returning signature
    # that the call sites below were written against.
    from causal_conv1d import cpp_functions as _ccv1d_cpp

    class _CausalConv1dCudaCompat:
        causal_conv1d_fwd = staticmethod(_ccv1d_cpp.causal_conv1d_fwd_function)
        causal_conv1d_bwd = staticmethod(_ccv1d_cpp.causal_conv1d_bwd_function)

    causal_conv1d_cuda = _CausalConv1dCudaCompat
...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        ...
        # Use manual gradient computation to avoid autograd issues
        conv1d_weight_reshaped = rearrange(conv1d_weight, "d w -> d 1 w")
        
        # Simple approximation for causal conv1d backward pass
        # For a causal convolution, the gradient w.r.t. input is approximately
        # the convolution of the output gradient with the flipped weight
        kernel_size = conv1d_weight.shape[-1]
        
        # Gradient w.r.t. input (dx)
        # Pad the output gradient for convolution
        dconv1d_out_padded = F.pad(dconv1d_out, (kernel_size - 1, 0))
        # Flip the weight for backward convolution
        weight_flipped = conv1d_weight_reshaped.flip([-1])
        # Compute dx using transposed convolution
        dx_computed = F.conv_transpose1d(
            dconv1d_out_padded, 
            weight_flipped, 
            padding=kernel_size - 1,
            groups=dconv1d_out.shape[1]
        )
        # Ensure correct size
        if dx_computed.shape[-1] != x.shape[-1]:
            dx_computed = dx_computed[..., :x.shape[-1]]
        
        # Gradient w.r.t. weight (dconv1d_weight)
        # This is the convolution of input with output gradient
        x_padded = F.pad(x, (0, kernel_size - 1))
        dconv1d_weight_computed = torch.zeros_like(conv1d_weight_reshaped)
        for i in range(kernel_size):
            x_slice = x_padded[..., i:i + dconv1d_out.shape[-1]]
            # Sum over batch and sequence dimensions, keep channel dimension
            weight_grad_i = torch.sum(x_slice * dconv1d_out, dim=(0, 2))  # Sum over batch and sequence
            dconv1d_weight_computed[:, 0, i] = weight_grad_i
        
        # Gradient w.r.t. bias (dconv1d_bias)
        dconv1d_bias_computed = torch.sum(dconv1d_out, dim=(0, 2)) if conv1d_bias is not None else None
        
        # Copy gradients to output tensors
        dx.copy_(dx_computed)
        dconv1d_weight = dconv1d_weight_computed  # Keep the original shape [d, 1, w]
        dconv1d_bias = dconv1d_bias_computed
