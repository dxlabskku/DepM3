...
class LMVDTriModal(data.Dataset):
    """LMVD dataset with 3 modalities: audio, visual, and text."""
    def __init__(
        ...
        with open(self.root / "labels.csv", "r") as f:
            for line in f:
                ...
                if self.is_sample(sample):
                    ...
                    t_feature_path = self.root / 'text' / f"{s_id}_text.npy"
                    ...
                    if t_feature_path.exists():
                        ...
                        # Assuming BERT pooled output (768 dims)
...
def get_lmvd_trimodal_dataloader(
    root: Union[str, Path], fold: str="train", batch_size: int=8, 
    gender: str="both",
    transform=None, target_transform=None, aug=True
):
    """Get dataloader for LMVD dataset with 3 modalities.

    Args:
        root (Union[str, Path]): path to the LMVD dataset.
    ...
    dataset = LMVDTriModal(root, fold, gender, transform, target_transform, aug)
...
if __name__ == '__main__':
    ...
    valid_loader = get_lmvd_trimodal_dataloader(
        "./data/lmvd", "valid"
    )
