...
# Default data paths
LMVD_DATA_PATH = os.environ.get("LMVD_DATA_ROOT", "./data/lmvd")
...
def parse_args():
    ...
    parser.add_argument("--use_pre_cossm_moe", type=lambda x: x.lower() in ['true', '1', 'yes'], default=True,
                       help="Whether to use Pre-CoSSM FiLM MoE")
    parser.add_argument("--pre_cossm_num_experts", type=int, default=5,
                       help="Number of experts for Pre-CoSSM FiLM MoE")
    parser.add_argument("--pre_cossm_top_k", type=int, default=2,
                       help="Top-k experts for Pre-CoSSM FiLM MoE")
    parser.add_argument("--pre_cossm_moe_loss_weight", type=float, default=0.01,
                       help="Weight for Pre-CoSSM MoE load balancing loss")
    
    # Modality dropout for missing modality robustness
    parser.add_argument("--modality_dropout_rate", type=float, default=0.15,
                       help="Probability of dropping each modality during training (0 to disable)")
    ...
    parser.add_argument("--pos_weight", type=float, default=0,
                       help="BCE pos_weight for class imbalance (0 = off)")
...
def train_epoch(
    ...
    with tqdm(
        ...
        for x, y, mask in pbar:
            ...
            # Post-CoSSM MoE load balancing loss
            ...
            pre_moe_loss = net.get_pre_moe_loss()
            ...
            # Total loss = classification + diversity + post-MoE + pre-MoE
            ...
            running_pre_moe_loss += pre_moe_loss.item() * x.shape[0]
            ...
            pbar.set_postfix({
                ...
                "pre_moe": running_pre_moe_loss / sample_count,
...
def main():
    ...
    print(f"Use Post-CoSSM MoE: {args.use_moe}")
    print(f"  Experts: {args.num_experts}, Top-k: {args.top_k_experts}")
    print(f"  Loss weight: {args.moe_loss_weight}")
    print(f"Use Pre-CoSSM FiLM MoE: {args.use_pre_cossm_moe}")
    print(f"  Experts: {args.pre_cossm_num_experts}, Top-k: {args.pre_cossm_top_k}")
    print(f"  Loss weight: {args.pre_cossm_moe_loss_weight}")
    print(f"--- Missing Modality Robustness ---")
    print(f"Modality dropout rate: {args.modality_dropout_rate}")
    print(f"---")
    ...
    for i_iter in range(args.num_iterations):
        ...
        drop_str = f"_drop{args.modality_dropout_rate}" if args.modality_dropout_rate > 0 else ""
        save_folder = f"multienc_{args.ensemble_stage}_{args.num_encoders}enc_{moe_str}{pre_moe_str}{drop_str}_{args.fusion_type}_{i_iter}_{timestamp}"
        ...
        print(f"Model type: {args.ensemble_stage.upper()} Ensemble + MoE (aware v2)")
        print(f"Post-CoSSM MoE: {args.use_moe}")
        if args.use_moe:
            print(f"  - {args.num_experts} experts per layer, top-{args.top_k_experts} activated")
        print(f"Pre-CoSSM FiLM MoE: {args.use_pre_cossm_moe}")
        if args.use_pre_cossm_moe:
            print(f"  - {args.pre_cossm_num_experts} experts, top-{args.pre_cossm_top_k} activated")
        ...
        if args.train:
            ...
            for epoch in range(args.epochs):
                ...
                if val_score < best_val_score:
                    best_val_score = val_score  # Update best validation score
                    epochs_without_improvement = 0
                    torch.save(
                        net.state_dict(),
        ...
        result_txt_path = os.path.join(
            ...
            f"{args.dataset}_multienc_{args.ensemble_stage}_{args.num_encoders}enc_{moe_str}{pre_moe_str}{drop_str}_{args.fusion_type}_{i_iter}_{timestamp}.txt"
        )
        with open(result_txt_path, 'w') as f:
            f.write(f"Multi-Encoder Ensemble + MoE (aware v2) Results\n")
            f.write(f"================================================\n")
            ...
            f.write(f"\nPre-CoSSM FiLM MoE Settings:\n")
            f.write(f"  Use Pre-CoSSM MoE: {args.use_pre_cossm_moe}\n")
            f.write(f"  Num Experts: {args.pre_cossm_num_experts}\n")
            f.write(f"  Top-K: {args.pre_cossm_top_k}\n")
            f.write(f"  Loss Weight: {args.pre_cossm_moe_loss_weight}\n")
            f.write(f"\nMissing Modality Robustness:\n")
            f.write(f"  Modality Dropout Rate: {args.modality_dropout_rate}\n")
            f.write(f"\nResults:\n")
