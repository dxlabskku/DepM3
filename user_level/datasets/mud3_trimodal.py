...
class MUD3TriModal(data.Dataset):
    """MUD3 dataset with trimodal compatibility.

    Each video is padded/truncated to FIXED_FRAMES (44) frames.
    Original 161-dim features are split into visual (136) and acoustic (25).
    Text modality is zero-filled for API compatibility with trimodal models.
    Per-sample output: visual (num_videos, 44, 136),
                       acoustic (num_videos, 44, 25),
                       text (num_videos, 44, text_dim).
    """
    def __init__(
        ...
        for _, row in label_df.iterrows():
            ...
            text = np.zeros(
                (visual.shape[0], FIXED_FRAMES, self.text_dim), dtype=np.float32
            )
            ...
            if self.aug and self.fold == "train":
                ...
                for _ in range(5):
                    ...
                    self.text_features.append(text[v_start:v_start + n_vids])

        print(f"[MUD3TriModal-{self.fold}] ALL:{len(self.labels)}, "
    ...
    def __getitem__(self, i: int):
        ...
        text = self.text_features[i]
        label = self.labels[i]
        return visual, acoustic, text, label
...
def _collate_fn(batch):
    visuals, acoustics, texts, labels = zip(*batch)

    max_num_videos = max(v.shape[0] for v in visuals)
    text_dim = texts[0].shape[2]
    ...
    for v, a, t in zip(visuals, acoustics, texts):
        n = v.shape[0]
        ...
        if n < max_num_videos:
            ...
            t_pad = np.zeros((max_num_videos - n, FIXED_FRAMES, text_dim), dtype=np.float32)
    ...
    padded_texts = torch.from_numpy(np.stack(padded_texts, axis=0))
    padding_masks = torch.from_numpy(np.stack(padding_masks, axis=0))
    labels = torch.tensor(labels)

    return padded_visuals, padded_acoustics, padded_texts, labels, padding_masks


def get_mud3_trimodal_dataloader(
    root: Union[str, Path], fold: str="train", batch_size: int=8,
    gender: str="both",
    transform=None, target_transform=None, aug=True
):
    """Get dataloader for MUD3 dataset with trimodal compatibility.
    ...
    Returns:
        the dataloader. Each batch returns:
            ...
            text:         (batch, max_num_videos, 44, text_dim)
            labels:       (batch,)
            padding_mask: (batch, max_num_videos) - 1 for real videos, 0 for padding
    """
    dataset = MUD3TriModal(root, fold, gender, transform, target_transform, aug)
...
if __name__ == "__main__":
    root = "./data/MUD3"
    train_loader = get_mud3_trimodal_dataloader(root, "train")
    print(f"train_loader: {len(train_loader.dataset)} samples")
    val_loader = get_mud3_trimodal_dataloader(root, "val")
    print(f"val_loader: {len(val_loader.dataset)} samples")
    test_loader = get_mud3_trimodal_dataloader(root, "test")
    print(f"test_loader: {len(test_loader.dataset)} samples")

    b = next(iter(train_loader))
    print(f"visual={b[0].shape}, acoustic={b[1].shape}, text={b[2].shape}, "
          f"labels={b[3].shape}, mask={b[4].shape}")
